...
const SignupPage = memo(() => {
  ...
  useEffect(() => {
    /\w+@gmail\.com/.test(emailInput) ? setEmailValidator(true) : setEmailValidator(false);
    ...
    <>
      ...
      {successLoginModal && (
        <Toast
          ...
          title="Signup Was Successfull"
        />
      )}
      {failedLoginModal && (
        <Toast icon={<MdError className="w-5 h-5 text-red-600" />} title="Signup Failed" />
      )}
    </>
  );
});
